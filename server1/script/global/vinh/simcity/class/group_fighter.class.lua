...
function GroupFighter:Add(tbNpc)
	...
	local nListId = self:_addNpcGo(tbNpc, 1)
	...
	if (tbNpc.rank ~= newRank) then
		if newRank > tbNpc.rank and SearchPlayer(tbNpc.playerID) == 0 then
			...
			if worldInfo.showThangCap == 1 then
				SimCityTongKim:announceRank(tbNpc.nMapId, tbNpc.hardsetName or SimCityNPCInfo:getName(tbNpc.nNpcId),
					newRank)
			end
		end
		tbNpc.rank = newRank
...
function GroupFighter:_respawnChild(nListId, childID, reason)
...
function GroupFighter:_isPlayerEnemyAround(nListId, nNpcIndex)
	...
	if GetNpcAroundPlayerList then
		...
		for i = 1, nCount do
			if (CallPlayerFunction(allNpcs[i], GetFightState) == 1 and IsAttackableCamp(CallPlayerFunction(allNpcs[i], GetCurCamp), tbNpc.camp)) then
				return 1
			end
		end
...
function GroupFighter:_joinFightCheck(nListId, nNpcIndex)
	...
	if (self:_isNpcEnemyAround(tbNpc, nNpcIndex, tbNpc.RADIUS_FIGHT_SCAN or RADIUS_FIGHT_SCAN) == 1) then
...
function GroupFighter:_leaveFightCheck(nListId, nNpcIndex)
	...
	if (self:_isNpcEnemyAround(tbNpc, nNpcIndex, tbNpc.RADIUS_FIGHT_SCAN or RADIUS_FIGHT_SCAN) == 0
...
function GroupFighter:_createChild(nListId, childID)
	...
	if not (child.isDead == 1 and tbNpc.noRevive == 1) then
		...
		nNpcIndex = AddNpcEx(child.nNpcId, 95, child.series or random(0, 4), nMapIndex, targetPos[1] * 32,
	...
	for i = 1, N do
		...
		if (child.finalIndex) then
			...
			if distance > checkDistance then
				...
				if tbNpc.arriveTick < tbNpc.tick and tbNpc.isFighting ~= 1 then
					self:_respawnChild(tbNpc.id, i, "Too far for 15 seconds")
					return 0
				end
...
function GroupFighter:_doParentTick(nListId)
	...
	if SearchPlayer(tbNpc.playerID) == 0 then
		...
		if tbNpc.walkMode == "random" or tbNpc.walkMode == "keoxe" or (not tbNpc.children) then
			if self:_arrived(nNpcIndex, tbNpc) == 1 then
			...
			local arrivedRes = self:_arrived(nNpcIndex, tbNpc)
		...
		if needRespawn == 1 then
			...
			tbNpc.nPosId = 1
			self:_generateWalkPath(tbNpc, 0)
			self:_respawn(tbNpc.id, 1, "keo xe qua map khac")
			return 1
		end
